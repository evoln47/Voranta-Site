...
## PENDING (do not forget): email deliverability - Gmail Promotions tab
The richer designed email now lands in Gmail Promotions (started after the email enrichment). Cause: promotional signals (large cyan button, multi-section table layout, List-Unsubscribe header). After the /100 build lands, do an api-engineer pass on api/_dri.js to favor Primary: keep the rich CONTENT but make presentation TRANSACTIONAL (lead with plain text, replace the big button with a simple text link, trim heavy table styling/branding); reconsider List-Unsubscribe on a 1:1 transactional reply. NOTE the tradeoff: richer/designed email vs Primary placement. Domain auth (SPF/DKIM/DMARC on the Resend domain) is the biggest lever and is an OWNER/DNS task, not code. Also: capture button "Sending..." bug was the FORM not hiding (css [hidden] override) + email actually sent fine - FIXED in styles.css 0bf043d + capture.mjs timeout 808f2e5.
