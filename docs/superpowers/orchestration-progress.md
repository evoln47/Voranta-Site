...
- WEIGHTS (sum 1.0): POV 0.30, Sensing 0.20, Conversion 0.20, Trust 0.16, Signal 0.14. DRI/100 = round(weighted sum of the five dimension values), round ONCE.
- ARCHETYPE: keep 4 archetypes + 2x2, RE-CUT axes to Upstream (mean of POV+Sensing fracs, high >= 0.70) x Downstream (mean of Conv+Trust+Signal fracs, high >= 2/3 - 1e-9). Cells: Publisher (up-high/down-low), Authority (up-high/down-high), Renter (up-low/down-low), Operator (up-low/down-high). Dimension-defined, coherent. Publisher/Authority blurbs now claim "strategy" (POV+Sensing) not POV-alone; cluster-silent guard extends to the 3 downstream dims.
- GAP/focus = lowest UNWEIGHTED dimension /100 (honest; weights drive DRI + axes, NOT gap selection). Tie-break funnel order: POV -> Sensing -> Conversion -> Trust -> Signal. deficit <75 / edge >=75. all-equal -> null + evenTier.
- RADAR becomes a PENTAGON (5 axes). Quadrant axis labels: "Strategy (Upstream)" x "Execution (Downstream)". buyer-dot: upstream mean (divider 0.70) x downstream mean (divider 2/3).
- RIPPLE: framework.mjs (5 dims, 5 questions w/ 6-opt scale, revised archetype blurbs), scoring.mjs (weighted sum, new axis thresholds, 5-dim focus), api/_dri.js (mirror + 5 dim rows + revised copy), scorecard.mjs ("All five dimensions"), scorecard-viz.mjs (RADAR_DIMS 5/pentagon, buyer-dot upstream/downstream, QUADRANT_PLACEMENT copy), test/scoring.test.mjs (enumeration 6^5=7776, retire split/per-dim helpers, CLAIMED_STRONG=upstream pair, new pairwise-independence + boundary + round-once tests, no-focus = 6 of 7776).
- TRADEOFF (acknowledged): single-item dimensions = no internal-consistency reliability (6-pt scale partially mitigates); much shorter quiz (<1 min) -> higher completion (serves booking). Replaces 12-question model.
- BUILD SEQUENCE (after in-flight agents land): framework-architect implements model+questions+scoring+_dri mirror+tests -> conversion-copywriter writes the 5 questions + all result copy FOLLOWING the readability rules (plain, NO "axis", no "the call is about") -> frontend-engineer pentagon radar + re-cut quadrant + the archetype-presentation redesign (from the visual proposal) -> api-engineer email for 5 dims -> review -> verify. Apply the ux CTA-placement + archetype-presentation proposals in this same build where they touch the results.
- EMAIL = PRESERVE the polished redesign (commit 7b36fba: masthead, dark score hero, cyan per-dimension SCORE BARS, accent-tint callout, structured footer, color-scheme:light). The 5-dim email step ADAPTS it (add the 5th dimension's row + score bar - bars are driven by dimensionScores /100 which still exist; add the Sensing DIMENSION_FIRST_SENTENCE; change "all four dimensions" -> "five"; carry revised plain copy). Do NOT rebuild the email from scratch or lose the masthead/dark-hero/score-bars.
- ARCHETYPE-PRESENTATION fix (visual proposal, fold into the frontend display step): ONE archetype description lives on Card 1 (score card). Card 3 (quadrant) drops the QUADRANT_PLACEMENT paragraphs -> a single-line position caption; add low/high axis ENDPOINT labels (e.g. "low POV"/"strong POV", "low exec"/"strong exec" - reframe to strategy/execution per the new model, plain, no "axis" word); tighten .sc-quadrant-callout sizing for one line. (Same redundancy also exists on aipq.html line 254 - fix in a follow-up.)

## ux AUDIT DONE: assessment-CTA placement (marketing funnel; separate workstream from the 5-q rebuild)
- BLOCKER: index.html hero ghost CTA "Take the 2-minute assessment" (lines 91-92) fires before comprehension. REMOVE from hero; keep "Book an intro call" primary; make the secondary a scroll anchor to #product (context-building, not trial).
- HIGH: add the assessment as a SECONDARY CTA at the closing sections of: index.html #contact (lines 270-278), aipq.html closing (lines 325-334, peak intent right after the sample scorecard), about.html closing (currently sponsor-only - strands the marketing-leader audience).
- MED: add an "Assessment" link to all four footers (.meta groups); consider nav label clarity (whose assessment).
- LOW: homepage section order is fine - do NOT reorder; the hero CTA position is the culprit.
- Note: verify the assessment.html static "Question 1 of 8" placeholder is overwritten at runtime (engine uses questions.length; will be 5 after rebuild).
- Implementation: frontend-engineer (CTA placements + footers) + conversion-copywriter (the secondary-CTA labels/framing) + visual-designer (secondary CTA treatment). Edits index/about/aipq + footers; coordinate with readability pass which touches assessment.html.

## QUEUED (human, high priority): assessment result copy READABILITY pass
Owner feedback: the assessment result copy feels abstract and hard to follow. Improve readability + comprehension; make it concrete and plain. HARD BANS (reader-facing copy): (a) the words "axis"/"axes" ENTIRELY - they read as internal scaffolding (e.g. "both axes need work", "the harder of the two axes to build", "one axis to close", "moving up the point of view axis", "you hold both axes"); reframe in plain terms. (b) the "the call is about / the call will be about" formula - DONE/removed already (commit d2ab484), keep it out. Also plainify abstract metaphors ("the lens", "the demand engine") where they hurt comprehension; consider whether "execution" reads clearly. The word "axis" lives mainly in scorecard-viz.mjs QUADRANT_PLACEMENT + api/_dri.js POSITIONING. Keep substance, the north-star (drive the booking), and ALL coherence constraints (cluster-silent: Operator/Authority must not claim a specific cluster dimension works; R2; no over-claims). Scope (all assessment result copy): framework.mjs archetype blurbs + the 8 dimension gap/edge blurbs; scorecard-viz.mjs QUADRANT_PLACEMENT (heavy "axis" language - rewrite); api/_dri.js LEAD_IN / POSITIONING / CTA_FRAMING (many "the call is about"); assessment.html CTA frame + intro. MAINTAIN framework.mjs <-> api/_dri.js blurb PARITY (the 2401 test enforces byte-identical) and run the cluster-silent guard + parity after. Do AFTER the in-flight display-swap (scorecard-viz.mjs) + question-polish (framework.mjs) agents land + commit, to avoid clobber.
