...
- Seven agent definitions exist in `.claude/agents/`, committed to the repo and available as spawnable subagent types.
...
- All seven use a consistent, structured output format (located finding + suggested fix) modeled on the existing `design-reviewer`.
...
| Decision | Choice |
|---|---|
| Roster size | Seven agents |
| Location | Project-level `.claude/agents/`, committed to the repo |
| Reviewer vs producer split | UX and design are read-only reviewers; copy, framework, frontend, api, and seo are producers that can edit |
...
Seven project-level agents in `.claude/agents/`. Boundary column states what each owns and, by implication, what it flags rather than touches.
...
| 2 | `design-reviewer` | Reviewer | Read-only | inherit | Design-system enforcement + visual hierarchy/type/spacing/layout advice |
| 3 | `conversion-copywriter` | Producer | Read + Edit/Write | inherit | The words — headlines, subheads, CTAs, microcopy, capture/result copy |
| 4 | `framework-architect` | Producer | Read + Edit/Write + Web | **Opus** | Research methodology, questions, scoring logic, archetypes |
| 5 | `frontend-engineer` | Producer | Full | inherit | Static HTML/CSS and vanilla-JS `.mjs` assessment implementation |
| 6 | `api-engineer` | Producer | Full | inherit | Vercel serverless functions, lead capture, env/secrets |
| 7 | `seo-engineer` | Producer | Read + Edit/Write | inherit | Titles, meta, OG/Twitter cards, JSON-LD, semantic HTML, sitemap/robots |
...
- **Design vs frontend on CSS:** design reviews and advises; frontend implements. Design is read-only and never edits CSS itself.
- **SEO vs frontend on markup:** seo owns head/meta/structured-data and semantic-element choices; frontend owns layout/styling/behavior. SEO edits head and semantic structure; it flags layout/visual concerns to design/frontend.
...
### 7.4 `framework-architect` (producer, Opus-pinned)
...
### 7.5 `frontend-engineer` (producer)
...
- **Default build sequence for a new section or feature:** `framework-architect` (if methodology is involved) → `conversion-copywriter` (words) → `frontend-engineer` / `api-engineer` (build) → `ux-reviewer` + `design-reviewer` in parallel (read-only review) → fixes → `seo-engineer` pass on any new/changed page.
- **Pre-commit convention:** run `ux-reviewer` and `design-reviewer` on any changed page before committing; run `seo-engineer` when a page is added or its content/structure changes.
...
- Seven `.claude/agents/*.md` files exist with correct frontmatter (`name`, `description`, `tools` where restricted, `model: opus` only on `framework-architect`).
- `design-reviewer.md` retains all ten original enforcement rules and adds the advisory half.
